...
namespace DTS_Wall_Tool.UI.Forms
{
    ...
    public class AutoLoadTab : UserControl
    {
        ...
        private void InitializeUI()
        {
            ...
            {
                Text = "Area Load (kN/m²) - Auto multiply by height",
                AutoSize = true
            };
            _rdoLineLoad = new RadioButton
            {
                Text = "Line Load (kN/m) - Direct value",
                AutoSize = true,
                Checked = true
            };
            ...
            // Sample modifier
            var modItem = new ListViewItem("ParapetHeight");
            modItem.SubItems.Add("1200");
            modItem.SubItems.Add("-");
            modItem.SubItems.Add("No");
        ...
        private void AddSampleStoryData()
        {
            var data = new[]
            {
                new { Story = "500", Type = "W200", Thick = "200", Height = "6500. 00" },
                new { Story = "7000", Type = "W200", Thick = "200", Height = "4500.00" },
                new { Story = "11500", Type = "W200", Thick = "200", Height = "3600.00" },
                new { Story = "15100", Type = "W200", Thick = "200", Height = "0.00" }
            };
        ...
        private void BtnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                var loadTable = LoadCalculator.GetQuickLoadTable(
                    (double)_nudParapetHeight.Value,
                    400  // beam height
                );

                string result = "Calculated Loads:\n\n";
                foreach (var item in loadTable)
                {
                    result += $"Wall {item.Key}mm: {item.Value:0.00} kN/m\n";
                }
        ...
        {
            MessageBox.Show("Assign loads to SAP2000 - Coming soon", "Info",
                MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
